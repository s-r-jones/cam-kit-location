...
void sc_GetBoneMatrix(thread float4 (&SPIRV_Cross_return_value)[3],thread const int& index,thread sc_SysIn& sc_sysIn,thread sc_SysOut& sc_sysOut,const constant sc_Set0& sc_set0,const constant sc_Set1& sc_set1)
{
int j=3*index;
float4 m[3];
m[0]=(*sc_set0.LibraryUniforms).sc_BoneMatrices[j+0];
m[1]=(*sc_set0.LibraryUniforms).sc_BoneMatrices[j+1];
m[2]=(*sc_set0.LibraryUniforms).sc_BoneMatrices[j+2];
SPIRV_Cross_return_value[0]=m[0];
SPIRV_Cross_return_value[1]=m[1];
SPIRV_Cross_return_value[2]=m[2];
}
...
int param=i;
float4 l9_0[3];
sc_GetBoneMatrix(l9_0,param,sc_sysIn,sc_sysOut,sc_set0,sc_set1);
float4 m[3];
m[0]=l9_0[0];
m[1]=l9_0[1];
m[2]=l9_0[2];
