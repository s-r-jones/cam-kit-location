...
vec4[3] sc_GetBoneMatrix(int index)
{
int j=3*index;
vec4 m[3];
m[0]=sc_BoneMatrices[j+0];
m[1]=sc_BoneMatrices[j+1];
m[2]=sc_BoneMatrices[j+2];
return m;
}
...
{
vec4 m[3];
m[0]=sc_GetBoneMatrix(i)[0];
m[1]=sc_GetBoneMatrix(i)[1];
m[2]=sc_GetBoneMatrix(i)[2];
